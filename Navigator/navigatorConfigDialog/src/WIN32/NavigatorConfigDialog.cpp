...
namespace
{
    Solipsis::NavigatorConfigDialog* dlg;  // This is a pointer to instance, since this is a static member
}

namespace Solipsis {

NavigatorConfigDialog::NavigatorConfigDialog()
{
    mHInstance = GetModuleHandle("NavigatorConfigDialog.dll");
...
INT_PTR NavigatorConfigDialog::DlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
#else
BOOL NavigatorConfigDialog::DlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
#endif
{
    HWND hwndDlgItem;
    ...
    case WM_INITDIALOG:
        ...
        pRend = lstRend->begin();
        // by default select the first rendering system
        if ((lstRend->size() > 0) && (dlg->mSelectedRenderSystem == 0))
            dlg->mSelectedRenderSystem = pRend[0];
        i = 0;
...
bool NavigatorConfigDialog::display(void)
{
    // Display dialog
    ...
    if (i == -1)
    {
        ...
        throw Exception(winError,errDesc, "NavigatorConfigDialog::display");
    }
    if (i)
        return true;
    else
