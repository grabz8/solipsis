														...
														PRInt32 curTotalProgress, PRInt32 maxTotalProgress )
{
	...
	// starting to update so it's okay to start grabbing a page
	mOkayToGrab = true;
...
NS_IMETHODIMP LLEmbeddedBrowserWindow::OnLocationChange( nsIWebProgress* aWebProgress,
														nsIRequest* aRequest,
															nsIURI* aLocation )
{
...
{
	// this method is exposed to app level so don't grab anything if we're in an invalid state
	if ( ! mOkayToGrab )
		return 0;
...
PRBool LLEmbeddedBrowserWindow::sendMozillaMouseEvent( PRInt16 eventIn, PRInt16 xPosIn, PRInt16 yPosIn )
{
	if ( ! mEnabled )
		return PR_FALSE;
...
PRBool LLEmbeddedBrowserWindow::sendMozillaKeyboardEvent( PRUint32 keyIn, PRUint32 ns_vk_code )
{
	if ( ! mEnabled )
		return PR_FALSE;
...
NS_IMETHODIMP LLEmbeddedBrowserWindow::HandleEvent( nsIDOMEvent* anEvent )
{
	...
	if ( linkElement )
	{
		...
		{
			LLEmbeddedBrowserWindowEvent event( getWindowId(), getCurrentUri(), mClickHref );
