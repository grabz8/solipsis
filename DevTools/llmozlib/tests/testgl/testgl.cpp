	...
	public LLEmbeddedBrowserWindowObserver
{
	public:
		...
		void init( char* arg0 )
		{
			...
			std::string profileName( mAppWindowName );
			LLMozLib::getInstance()->init( profileBaseDir, profileName );
			mBrowserWindowId = LLMozLib::getInstance()->createBrowserWindow( getNativeWindowHandle(), mBrowserWindowWidth, mBrowserWindowHeight );
