	...
	mouseButtonRDown(false), zOrderCounter(1), keyboardHook(0)
{
    ...
    // GILLES : remove the working default directory to enable a full path description later for each FlashControl
    this->basePath = "";
}
...
FlashControl* HikariManager::createFlashOverlay(const Ogre::String& name, Ogre::Viewport* viewport, int width, int height, const Position& position, Ogre::uchar zOrder, Ogre::uchar tier)
{
    ...
    // GILLES BEGIN : add the base directory if 'name' describe only a filename
    std::string workingDirectory("");
    //if( name.find(":") == std::string::npos )
    //    workingDirectory = getCurrentWorkingDirectory();// + "solTmpTexture\\";
    // GILLES END

    return controls[name] = new FlashControl(workingDirectory + name, viewport, width, height, position, zOrder, tier);
}
...
FlashControl* HikariManager::createFlashMaterial(const Ogre::String& name, int width, int height)
{
    ...
    return controls[name] = new FlashControl(workingDirectory + name, width, height);
}
