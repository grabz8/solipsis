...
bool PhysXHelpers::init(NxUserOutputStream* outputStream)
{
    ...
    rd->createObject(sdk, NX_DBG_OBJECTTYPE_CAMERA, "Camera 1", NX_DBG_EVENTMASK_EVERYTHING);
    rd->writeParameter(NxVec3(0.0f, 20.0f, 0.0f), sdk, true, "Origin", NX_DBG_EVENTMASK_EVERYTHING);
    rd->writeParameter(NxVec3(0.0f, 0.0f, 0.0f), sdk, true, "Target", NX_DBG_EVENTMASK_EVERYTHING);
    rd->writeParameter(NxVec3(1.0f, 0.0f, 0.0f), sdk, true, "Up", NX_DBG_EVENTMASK_EVERYTHING);
#endif
...
bool PhysXHelpers::shutdown()
{
    ...
    NxPhysicsSDK* sdk = PhysXHelpers::getNxPhysicsSDK();
    NxRemoteDebugger* rd = sdk->getFoundationSDK().getRemoteDebugger();
    rd->removeObject(sdk, NX_DBG_EVENTMASK_EVERYTHING);
    rd->disconnect();
#endif

    NxReleasePhysicsSDK(mNxPhysicsSDK);
    mNxPhysicsSDK = 0;
...
ofs.close();*/
    ...
    NxTriangleMesh* triangleMesh = mNxPhysicsSDK->createTriangleMesh(MemoryReadBuffer(buf.data));
    NxCloseCooking();
