...
{
    OGRE_LOG("NavigatorGUI::login() lock");
    pthread_mutex_lock(&mNavigator->mNaviMutex);
    // Hide previous Navi UI
    hidePreviousNavi();
    ...
    if (mNavisStates[NAVI_LOGIN] == NSNotCreated)
    {
        ...
        mNaviMgr.createNavi(mNavisNames[NAVI_LOGIN], "local://uilogin.html", NaviPosition(Center), 400, 300, false, true);
    ...
    OGRE_LOG("NavigatorGUI::login() unlock");
    pthread_mutex_unlock(&mNavigator->mNaviMutex);
}
...
void NavigatorGUI::loginPageLoaded(const NaviData& naviData)
{
    ...
    OGRE_LOG("NavigatorGUI::loginPageLoaded() lock");
    pthread_mutex_lock(&mNavigator->mNaviMutex);
    ...
    OGRE_LOG("NavigatorGUI::loginPageLoaded() unlock");
    pthread_mutex_unlock(&mNavigator->mNaviMutex);
}
...
void NavigatorGUI::options(const NaviData& naviData)
{
    ...
    OGRE_LOG("NavigatorGUI::options() lock");
    pthread_mutex_lock(&mNavigator->mNaviMutex);
    // Hide previous Navi UI
    hidePreviousNavi();
    ...
    if (mNavisStates[NAVI_OPTIONS] == NSNotCreated)
    {
	    ...
	    mNaviMgr.bind(mNavisNames[NAVI_OPTIONS], "ready", NaviDelegate(this, &NavigatorGUI::optionsPageLoaded));
    ...
    OGRE_LOG("NavigatorGUI::options() unlock");
    pthread_mutex_unlock(&mNavigator->mNaviMutex);
}
...
void NavigatorGUI::optionsPageLoaded(const NaviData& naviData)
{
    ...
    OGRE_LOG("NavigatorGUI::optionsPageLoaded() lock");
    pthread_mutex_lock(&mNavigator->mNaviMutex);
    // Set current values
    ...
    OGRE_LOG("NavigatorGUI::optionsPageLoaded() unlock");
    pthread_mutex_unlock(&mNavigator->mNaviMutex);
}
