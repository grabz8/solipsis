...
int ReplicaManager2::Replica2CompByNetworkID( const NetworkID &key, Replica2 * const &data )
{
...
int ReplicaManager2::Replica2ObjectComp( Replica2 * const &key, Replica2 * const &data )
{
...
int ReplicaManager2::Connection_RM2CompBySystemAddress( const SystemAddress &key, Connection_RM2 * const &data )
{
...
PluginReceiveResult ReplicaManager2::OnSerialize(unsigned char *packetData, int packetDataLength, SystemAddress sender, RakNetTime timestamp)
{
	...
	if (replica)
	{
		...
		AddConstructionReference(connection, replica);
// GREG BEGIN
        AddVisibilityReference(connection, replica); // GREG
// GREG END
	}
	return RR_STOP_PROCESSING_AND_DEALLOCATE;
}
...
// GREG BEGIN
	{ // GREG
		clientPtrArray[Replica2::clientSharedID]=this; // GREG
...
void Replica2::ReceiveSerialize(SystemAddress sender, RakNet::BitStream *serializedObject, SerializationType serializationType, RakNetTime timestamp, DataStructures::OrderedList<SystemAddress,SystemAddress> &exclusionList )
{
	...
	{
// GREG BEGIN
		if (!rm2->connectionList[i]->ReplicaIsConstructed(this)) //GREG
		    continue; //GREG
// GREG END
...
Replica2 * Replica2::ReceiveConstructionReply(SystemAddress sender, RakNet::BitStream *replicaData, bool constructionAllowed)
{
	(void) replicaData;
	(void) sender;
...
// GREG BEGIN
					if (obj->QueryIsConstructionAuthority()) // GREG
					{ // GREG
						BooleanQueryResult bqr=obj->QueryConstruction(replicaManager->connectionList[i]); // GREG
						if (bqr==BQR_ALWAYS || bqr==BQR_YES) // GREG
						{ // GREG
...
// GREG BEGIN
bool Connection_RM2::ReplicaIsConstructed(Replica2* replica) const // GREG
{ // GREG
	return lastConstructionList.HasData(replica); // GREG
} // GREG
// GREG END
