// Plugin_3ds.cpp : Defines the exported functions for the DLL application.
...
const String sPluginName = "Plugin_3ds";
//------------------------------------------------------
Plugin_3ds::Plugin_3ds()
{
}

//------------------------------------------------------
const Ogre::String& Plugin_3ds::getName() const
{
	return sPluginName;
}
//------------------------------------------------------
    
void Plugin_3ds::install()
{
}
//------------------------------------------------------
void Plugin_3ds::initialise()
{
}
//------------------------------------------------------
void Plugin_3ds::shutdown()
{
}
//------------------------------------------------------
void Plugin_3ds::uninstall()
{
}
//------------------------------------------------------
#define PRINT_ERRORS_RETURN(file) {if(ftkerr3ds){DumpErrList3ds(file); if (!ignoreftkerr3ds) return (false);}}

Ogre::String Plugin_3ds::ReplaceSpaces(const Ogre::String& s)
{
	Ogre::String res(s);
...
//-------------------------------------------------------------------------------------
bool Plugin_3ds::FolderExist(std::string strPath)
	{
		WIN32_FIND_DATA   wfd;
		bool rValue = false;
...
//-------------------------------------------------------------------------------------
bool Plugin_3ds::convert3dsToMesh(const Ogre::String & sz3dsFileName,
						...
						bool bTextureVerticalFlip)
{
    ...
    ifile = OpenFile3ds(sz3dsFileName.c_str(), "r");
...
#define TEXCOORD_BINDING 2
			...
			if(mesh->ntextverts == 0)
				newDecl->removeElement(Ogre::VES_TEXTURE_COORDINATES);
  ...
  //      if(mat->ambient.r != 0||mat->ambient.g != 0||mat->ambient.b !=0)
		...
		else if(mat->shading == Gouraud)
			ogremat->setShadingMode(Ogre::ShadeOptions::SO_GOURAUD);
		else if(mat->shading == Phong)
			ogremat->setShadingMode(Ogre::ShadeOptions::SO_PHONG);
		//ogremat->setLightingEnabled(false);
											 ...
											 Ogre::SceneManager* mSceneMgr)
{
 ...
 //   //Ogre::LogManager::getSingleton().createLog("3DSOgreExporter.log", true);
	...
	//assert(LogMgr != NULL);
	//LogMgr->logMessage("OGRE PLUGIN_3DS Log");
	//LogMgr->logMessage("---------------------------");
	
	Ogre::String szMeshPrefix = sz3dsFileName.substr(0,sz3dsFileName.length()-4);

	

	size_t lastSlash = sz3dsFileName.find_last_of("\\");
	Ogre::String mResourceFloder = sz3dsFileName.substr(0,lastSlash+1);
	...
	convert3dsToMesh(sz3dsFileName,szMeshPrefix);
