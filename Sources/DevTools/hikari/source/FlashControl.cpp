...
void FlashControl::defocus()
{
    HikariManager::Get().defocusAll();
}
...
void FlashControl::handleFlashCall(const std::wstring& xmlString)
{
	...
	if(i != delegateMap.end())
	{
		 ManagedReferenceHolder MRef;
         MRef.NativeArguments = args;
         MRef.FuncName = funcName; 

		FlashValue retval = i->second(this, MRef);
