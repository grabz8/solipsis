...
bool uBrowser::init( const char* arg0, int appWindowIn )
{
	...
	std::string profileName( "uBrowser" );
	...
	for( int i = 0; i < mNumBrowserWindows; ++i )
	{
		mWindowId[ i ] = LLMozLib::getInstance()->createBrowserWindow( getNativeWindowHandle(), mBrowserWindowWidth, mBrowserWindowHeight );
						...
						0, 0,
							...
							LLMozLib::getInstance()->getBrowserRowSpan( mWindowId[ i ] ) / LLMozLib::getInstance()->getBrowserDepth( mWindowId[ i ] ),
								mBrowserWindowHeight,
									LLMozLib::getInstance()->getBrowserDepth( mWindowId[ i ] ) == 3 ? GL_BGR_EXT : GL_BGRA_EXT,
										GL_UNSIGNED_BYTE,
											pixels );
				};
