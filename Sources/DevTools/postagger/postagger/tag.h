...
int init( const std::string& filepath, std::vector< ME_Model >& vme ) 
{
  for (int i = 0; i < vme.size(); i++) {
    char buf[1024];
#ifdef WIN32
	sprintf_s( buf, "%s/model.bidir.%d", filepath.c_str(), i );
#else    
	sprintf( buf, "%s/model.bidir.%d", filepath.c_str(), i );
#endif
...
int tag( const std::string& text, const std::vector< ME_Model >& vme, std::string& taggedtext )
{
  //int para = -1;
  ...
  //vector<ME_Model> vme(16);
	...
	taggedtext = bidir_postag( text, vme );
