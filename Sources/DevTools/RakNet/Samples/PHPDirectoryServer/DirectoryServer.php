...
function update_game_list() {
    ...
    $whole = fread($input,$gv['max_length']);
    if(! feof($input))
        trigger_error("Updated record exceeds max size", E_USER_WARNING);
    $parts = explode("\001", $whole);

    error_log("whole" . $whole);
    // apply updates
    foreach($parts as $part) {
        list($akey, $avalue) = array_map('trim',explode("\002",$part,2));
