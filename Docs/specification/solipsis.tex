...
An avatar node wishing to join the \sol world should first select a
joining position. This should in general be the last position of the
avatar in the \sol world but may also be any other position, as in the
case of an avatar joining for the first time. The actual joining
operation simply consists in routing an \avhb message to this joining
position.

\toadd JOIN OPERATION FOR SITE NODES
...
%    \If {old = null \lor old.\seqNum \lt \avhb.nd.\seqNum}
%        \vlist.put(\avhb.nd.\UID,\avhb.nd)
%    \EndIf
