...
with a nodes in the \ray overlay characterized by virtual coordinates
that are the same as those of its site in the 3D world. If a host owns
multiple sites in the 3D world, then it appears in the \ray overlay as
...
strategies exploited by \ray. On the other hand, it allows the overlay
...
their nerighbors in the \ray overlay. Receiving nodes, process this
...
It is important to observe that the position of a node in the \ray
...
possible that the \ray cell associated with a node comprises a region
...
reasons. First, \ray only considers the neighborhood relationship
