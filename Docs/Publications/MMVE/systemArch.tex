...
each consisting of a mesh or prims based model, a set of textures, and, in the
case of avatars or objects, also of an animation. \df{Are there animations for
  objects and sites????} \jr{jerome :yes for objects, we can associated an animation, that will be compute by a host if he want to manage it. Also, the object can be take by a avatar, in this case the client hosting the avatar have to manage it (if nobody already animate it, and if he decides to animate it. If nobody decide to manage this animation, the site takes the responsability to manage this object node, but doesn't animate it}
...
computation and communication cost \jr{You can remove this sentence, as nobody really used it in the related works. World of warcraft and co just transmit the position. You can write something like "`\bf{The state in the universe of an entity is defined thanks to a \emph{entity descriptor}}"' } For this reason, each entity is
...
interaction with the rest of the metaverse. \jr{For example a key of animation, corresponding to a state of the animation, such as an index doing reference to a previously transmitted  table of all positions of the body defined by the angles of each joint of the skelet. This could be compressed, how ??? Today it's a good question, we have just to wait that standards integrate them !!!}
...
information about the corresponding entity's descriptor as well as the
...
navigator platform.\df{Anything else to say about the node??}
...
descriptions and to offer to other  \sol nodes running on the same or
remote hosts. 

To achieve this, we store the 3D-descriptions managed or downloaded by
the nodes residing on a given host, in a common data repository that
...
the files that constitute the object's 3D description. 

tight integration with the corresponding entities' nodes,
3D-descriptions are not stored individually by each node, but, rather,
they are stored in a common data repository shared by all the nodes
running on the same \sol host. This allows multiple nodes running on
the same host to access a common set of 3D description without having
to download multiple copies of the same data. Whenever a node caches a
...
the new copy. Similarly, when a node detects that a host listed in the
