...
capabilities of a host.~\cite{cavagna}.\df{did some editing please
  check}
% auto-regulate the network load
Table~\ref{Table:descriptor} shows a sample descriptor containing the
...
between the corresponding points this space. Specifically, two Raynet
...
management, ans starts computing its physics, and animation, and
...
well as those of other visible objects in a shared repository that is
accessible by all the nodes running on the same host. \jr{ finally,
  why not call that "`State Node description"' or "`State
  description"' instead of "`3D-description"' ???}\df{State
  description sounds like the descriptor... this is the model, texture
  and animation} Moreover, each node records in its entity's
descriptor a list of the hosts that currently hold a copy of any of
the files that constitute the associated 3D description.  Whenever a
...
updates the descriptor accordingly.\jr{I think that a figure could
  simply the understanding}
