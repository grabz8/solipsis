...
modifications. The two levels we consider consist of a \emph{complete
  description} of the object and of a an \emph{object
  descriptor}\df{This will change. Ultimately we are going to have:
  descriptor, and 3dData, which in turn consists of mesh, texture, and
  animation.}. The former contains all the information necessary for
the 3D visualization of an object, plus any sounds, videos, or
multimedia content associated with it. From the point of view of this
protocol, such a representation merely consists of a binary object
that should be provided to the rendering component and of an
associated version number. The object descriptor, on the other hand,
consist of the minimal information required to display a rough
representation of the object and includes the object's location, its
shape, its size, and its visibility radius. The shape of the object is
