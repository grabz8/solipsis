...
specified object descriptor. If the \vlist contains no entry for the
specified object, then the received descriptor is simply inserted in
it. If, on the other hand, the \vlist already contains entry for the
object, the receiving node replaces it with the received one only if
the latter is associated with a fresher sequence number.
